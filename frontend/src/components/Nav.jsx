...
import FoodZoneLogo from '../assets/FoodZone.png'
...
function Nav({ isDashboard }) {
    ...
    return (
        <div className={`w-full ${isDashboard ? 'h-[70px] static' : userData?.role === 'customer' ? 'h-[70px] sticky top-0' : 'h-[80px] fixed top-0'} flex items-center justify-between md:justify-center gap-[30px] px-[20px] z-[90] bg-[#fff9f6] overflow-visible shadow-sm`}>

            {showSearch && userData.role == "customer" && <div className='w-[90%] h-[70px]  bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden px-4'>
            ...
            <img src={FoodZoneLogo} alt='Prime Dine Logo' className='h-[55px] w-auto object-contain cursor-pointer' onClick={() => navigate('/')} />
            {userData.role == "customer" && <div className='md:w-[60%] lg:w-[40%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] hidden md:flex px-4'>
                <div className='w-full flex items-center gap-[10px]'>
                    <IoIosSearch size={25} className='text-[#ff4d2d]' />
                    <input type="text" placeholder='search delicious food...' className='px-[10px] text-gray-700 outline-0 w-full' onChange={(e) => setQuery(e.target.value)} value={query} />
                </div>
            ...
            <div className='flex items-center gap-4'>
                ...
                {userData.role == "admin" && !isDashboard ? <>
                ...
                {showInfo && <div className={`fixed top-[80px] right-[10px] 
                    ${userData.role === "cook" ? "md:right-[20%] lg:right-[40%]" : "md:right-[10%] lg:right-[25%]"} w-[180px] bg-white shadow-2xl rounded-xl p-[20px] flex flex-col gap-[10px] z-[9999]`}>
